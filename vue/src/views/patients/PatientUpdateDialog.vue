...
watch(() => props.visible, (newVal) => {
  if (newVal && props.patient) {
    form.value = {
      ...
      dateNaissance: props.patient.dateNaissance ? props.patient.dateNaissance.split('T')[0] : '',
      telephone: props.patient.telephone || '',
      email: props.patient.email || '',
      adresse: props.patient.adresse || '',
      antecedentsMedicaux: props.patient.antecedentsMedicaux || '',
      groupSanguin: props.patient.groupSanguin || ''
    }
  }
}, { immediate: true })

const handleSubmit = () => {
  emit('save', { id: props.patient.id, ...form.value })
}
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="space-y-1">
            ...
            <input 
              ...
              type="date" 
              class="w-full py-2 px-3 text-xs bg-slate-50 border border-slate-200 focus:border-sky-500 focus:ring-1 focus:ring-sky-550 rounded-lg outline-none text-slate-800"
            />
          </div>
          <!-- Téléphone -->
          <div class="space-y-1">
